...
class SpoofingNet(nn.Module):
    def __init__(self,
        ...
        self.MBConv[-1] = torchvision.models.mobilenet.ConvBNReLU(in_planes=320,
                                                                  out_planes=mb_out_channels,
                                                                  kernel_size=1,
                                                                  stride=1)
        self.MBConv.requires_grad_(not mb_fixed)
        self.Avgpool = nn.AdaptiveAvgPool2d(output_size=pool_out)
        self.Conv1x1 = nn.Conv2d(in_channels=mb_out_channels * num_samples,
        ...
        self.Linear = nn.Linear(in_features=feat_out_channels,
