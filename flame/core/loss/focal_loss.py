...
class FocalLoss(loss.LossBase):
    def __init__(self, alpha: float = 0.5, gamma: float = 2.0,
                 reduction: str = 'mean', output_transform=lambda x: x) -> None:
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        target: torch.Tensor = self._onehot_encode(labels=target, n_classes=pred.shape[1], device=pred.device, dtype=pred.dtype)
